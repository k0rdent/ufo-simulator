...
# workflow run on main (same pattern as k0rdent/core-services e2e-periodic-ufo).
set -euo pipefail
...
for attempt in $(seq 1 "$max_attempts"); do
  echo "Attempt $attempt/$max_attempts for $TARGET_REPO ($WORKFLOW_ID)"
  rm -vf "$outdir/chart_version.txt" "$outdir/image_tag.txt"

  RUN_ID=$(gh api \
    "repos/$TARGET_REPO/actions/workflows/$WORKFLOW_ID/runs?branch=main&status=success" \
...
echo "All attempts failed for $TARGET_REPO"
exit 1
