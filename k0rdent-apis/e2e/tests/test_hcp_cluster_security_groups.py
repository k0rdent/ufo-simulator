"""HCP cluster + VPC/cluster security-group binding, NICo/UFO materialization,
and the security-groups-effective read (KNF-469).

The effective read is asserted here rather than in a scenario of its own because
this is the only place a REAL owner-level binding exists: the upstream MOCK_MODE
suite cannot create a cluster, so ``objectKind=cluster`` — the merge arm — and the
agreement between the read and what NICo actually renders are untestable there.
Standing up a second cluster to check them would double a 30-minute run.
"""
...
def test_hcp_cluster_vpc_security_groups(
    ...
    and the effective read at every binding stage."""
    ...
    cluster_sg = secgroups.ensure_security_group(
    ...
    secgroups.fresh_resource(session, clusters_url, cluster, what="cluster")
    ...
    vpcs = secgroups.owner_vpcs(
        session, api_base, region, project, owner_kind="cluster", owner_uid=cluster_uid
    )
    ...
    if effective_ready:
        ...
        eff = _effective("cluster", cluster_id)
        secgroups.assert_effective_object(
            eff, kind="cluster", obj_id=cluster_id, uid=cluster_uid
        )
        assert secgroups.effective_group_ids(
            secgroups.effective_block(eff, vpc_id)
        ) == [default_sg_id], (
            "the cluster binding is still empty, so only the VPC floor applies"
        )
        ...
        log.ok(
            f"{len(eff.get('vpcs') or [])} block(s); non-nico blocks empty"
        )
    ...
    def _await_effective_matches_nsg(label: str) -> None:
        ...
        def _pred():
            ...
            if not fresh:
                return None
            block = secgroups.effective_block(_effective("cluster", cluster_id), vpc_id)
    ...
    cluster_rule_names = {
        r.get("name")
        for r in secgroups.sg_api_rules(sgs_by_id[cluster_sg_id])["ingress"]
        if r.get("name")
    }
    vpc_rule_names = {
        r.get("name")
    ...
    def _nsg_has_cluster_and_precedence():
        ...
        have = secgroups.nsg_rule_fingerprints(fresh)
        want = (
            secgroups.api_rule_fingerprints(sgs_by_id[cluster_sg_id])
        ...
        cluster_idx = secgroups.first_index(order, cluster_rule_names)
    ...
    cluster_fps = secgroups.api_rule_fingerprints(sgs_by_id[cluster_sg_id])
    ...
    if effective_ready:
        log.step("effective read: cluster binding merges ahead of the VPC floor")
        want_ids = [cluster_sg_id, vpc_custom_sg_id, default_sg_id]
        ...
        block = wait.await_predicate(
            ...
            timeout=900,
            interval=10,
            desc=f"effective cluster block for {vpc_id} == {want_ids!r}",
            steps=log,
            log_every=2,
        )
        secgroups.assert_effective_group(
        ...
        have = secgroups.effective_rule_fingerprints(block)
        want = cluster_fps | vpc_custom_fps | default_fps
        assert have == want, (
        ...
        _await_effective_matches_nsg("cluster + VPC bindings")
        log.ok()
    ...
    try:
        ...
        if effective_ready:
            log.step("effective read: cluster SG gone from the block")

            def _effective_without_cluster_sg():
                block = secgroups.effective_block(
                    _effective("cluster", cluster_id), vpc_id
                )
                ...
                if secgroups.effective_rule_fingerprints(
                    block, security_group_id=cluster_sg_id
                ):
                    return None
                return block

            wait.await_predicate(
                _effective_without_cluster_sg,
                timeout=900,
                interval=10,
                desc=f"effective cluster block dropped {cluster_sg_id}",
                steps=log,
                log_every=2,
            )
            log.ok()
        ...
        if effective_ready:
            ...
            wait.await_predicate(
                ...
                timeout=900,
                interval=10,
                desc=f"effective cluster block for {vpc_id} == [{default_sg_id}]",
                steps=log,
                log_every=2,
            )
            log.ok()
        ...
        if effective_ready:
            log.step(f"effective read: deleted cluster {cluster_id} is 404")
            gone = secgroups.get_effective_raw(
                session,
                api_base,
                region,
                project,
                params={"objectKind": "cluster", "objectId": cluster_id},
            )
            assert gone.status_code == 404, (
                f"effective read of a deleted cluster must be 404, got "
        ...
        secgroups.delete_security_group(session, sg_collection, cluster_sg_id, log=log)
